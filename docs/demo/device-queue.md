...
| 41 | Ira's cream switch | You → Appearance → Cream: every screen cream, status icons dark; kill and reopen: still cream; back to Dark | `ThemePreference` through `LocalScheme`, the bars' icon colour at run time | LANDED AND UNPROVEN (Ira, 12:50) |
| 38 | Ira's language page | You → Language → Telugu: the sheet's label reads "LISTENING · తెలుగు" on the next hold | the language's one home (the profile row) read by both screens | LANDED AND UNPROVEN (Ira, 12:50) |
