...
RUN LOG 22 Sep 14:14-15:00 (master 326ed6a + logout, realme): row 1 install PASS (3 tries, link
blip), migration v2->v4 on real rows PASS (Today: chapatiis 232.3 kcal 6:43 am); 3a pre-flight WAIT
on loadavg (this phone idles at 15-20, phantom D-state; runnable 1 is its signal) and ran anyway;
row 5 Beat 1 by microphone FAIL x2 at the recogniser (mangled transcripts; the chain ran; the
phone was being handled: recorder open, a meal logged by hand 2:27 PM, shade, then locked); row 4
typed Hindi rows: row 1 PASS (frozen Beat 1 -> roti + dal, the first Hindi plate on a device),
row 2 PASS (roti, dal, tel), row 5 stalled mid-phrasing. THE STALL'S CAUSE, MEASURED: behind the
lock screen, the shade, the recorder or any other focused window the process sits in cpuset
/background = cores 0-3 (little), and 8 llama threads there collapse (2-token prompt 14,864 ms
vs 348 ms in front); the rig has no window and lives there. 66 s, 83 s, 108 s and every "0 % CPU"
were this. Phone dropped off the cable 15:00 (unlock attempt); rows 5, 5a, 36, 37, Ira's 38-47,
1-3, 7 NOT RUN. Next window resumes at row 5 with the phone UNLOCKED and IN2FIT in front.

Conditions before item 1, every time: the phone UNLOCKED, IN2FIT the focused window, the shade
closed, DND on (the pre-flight now checks lock screen, focused window and the app's cpuset), default adb server only (`adb devices` lists the phone,
...
| 2 | Task 1, the same utterance through the rig, same session — ONLY IF 1 REPRODUCES; NOTE 22 Sep: the rig has no window and runs in cpuset /background (little cores); its numbers are little-core numbers until the rig is given a foreground cpuset (the report now prints the cpuset) | `OrchestratorDeviceTest.c_oneHindiAnswer` (typed, no UI) twice | fast in the rig + slow in the UI = the UI or its threads (Ira's too) | UNPROVEN |
...
| 42 | Ira's Coach, typed, (g) | Coach: type a question, Send: the stage rows appear where the design has typing dots, then the answer; after Beat 3, the first chip "Advise again on my last meal" runs Beat 4 on the logged meal | the Coach input feeds `type()`; the chip feeds `adviseAgain()` | LANDED AND UNPROVEN (Ira, 11:15) |
...
| 53a | Log out (Rao, 22 Sep, on request) | You -> Log out -> the dialog -> Log out: every table wiped (`clearAllTables`), Welcome shows again; Cancel leaves everything | the phone forgets the person; there is no account | LANDED AND UNPROVEN |
