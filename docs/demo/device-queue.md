...
| 43 | Ira's warm-up gate on the v2 bar | a hold within the first second after launch: nothing starts and the button is dim; a hold after the warm-up line starts the sheet | `state.ready` on the v2 microphone | LANDED AND UNPROVEN (Ira, 13:20) |
...
| 45 | Ira's Discard | after a spoken LOG: Discard on the sheet; Diary and the next ANSWER no longer carry the meal | the sheet's Discard on `deleteMealAndAdvice` | LANDED AND UNPROVEN (Ira, 11:55) |
...
| 49 | Ira's water | "+" on the water card twice: 0.5 appears; kill and reopen: still 0.5 | `logWater` through the v2 card | LANDED AND UNPROVEN (Ira, 12:25) |
...
| 56 | Ira's share sheet | You → Privacy → "Share with your doctor": the chooser opens with the CSV text, no permission prompt | the export through the v2 page | LANDED AND UNPROVEN (Ira, 12:50) |
