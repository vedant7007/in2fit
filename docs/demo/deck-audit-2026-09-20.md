...
| deck says | repository says | verdict |
| --- | --- | --- |
| "One sentence, three languages. Recognition runs on the handset, tuned for how people here actually mix Telugu, Hindi and English mid-sentence." | `0022-one-model-for-mixed-speech`, measured today: no available model transcribes code-mixed speech cleanly; the app runs ONE checkpoint per the language the profile selects (option A), and "A does not literally meet 'one model for any mix', and nobody is pretending it does". Language is a setting, never detected (`0015`, spec 10.2). A Telugu-profile user's Hindi sentence comes back as Telugu-script transliteration; Priya's `0041-code-mixed-food-words` recovers English food words said the Telugu way inside a Telugu sentence. | **FIX FIRST. UNSUPPORTED as worded.** Say instead: "Recognition runs on the handset in the language you choose, and English food words mixed into that language are understood". Do not say "tuned for mixing three languages mid-sentence". |
...
## Hard problem 01, third wording, as dictated by Vedant (23:17); the PDF re-read when it lands

The personal katori is off the deck: nothing calls that table. The card now reads, in Vedant's
words: *"Every house has a different bowl, so a katori is not a unit. The food itself decides what
a bare number means: two roti is two rotis, one katori dal is a serving whose grams we supply. And
when the amount was ours rather than yours, it never hides inside the total. It travels beside it,
as 'one katori, taken as 180 g'."* Checked against `LookupMealResolver` at `425691d` and `0035`.

| sentence | the code | verdict |
| --- | --- | --- |
| "a katori is not a unit" | `household-units.csv` carries the katori per food class (150 g for a cooked pulse or grain, 100 g for a sabzi), and a served dish takes its recipe's serving instead (dal tadka 180 g); `resolveUnit` is keyed on unit AND class. | RIGHT |
| "The food itself decides what a bare number means: two roti is two rotis" | `weigh()`: COUNTED by the recipe's moisture class (GRIDDLE_BREAD, DEEP_FRIED, STEAMED, EGG, MIXED_PLATE); "two rotis" is 2 pieces, QUANTITY_STATED, 90 g (`UnstatedQuantityTest`). | RIGHT |
| "one katori dal is a serving whose grams we supply" | If the person SAYS katori, the unit is theirs: HOUSEHOLD_UNIT_DEFAULT, Approximate, grams the recipe's serving, and the card shows "1 katori (180 g)" with no "taken as" (`a7495a3`). A BARE number, "one dal", is the case the sentence is about: assumed katori, QUANTITY_INFERRED, Rough, "taken as 180 g". | RIGHT in substance; the example is not a bare number. "one dal is a serving whose grams we supply" would match the code's case exactly; as written it is still true (we supply the grams either way). Not a fix-first. |
| "when the amount was ours rather than yours, it never hides inside the total. It travels beside it, as 'one katori, taken as 180 g'" | Resolver: RIGHT. `Assumed(quantity, unit)` is written back onto the item; "taken as" only when QUANTITY_INFERRED (`a7495a3`, `0035`). Screen at `67a6dec`: **NOT RENDERED.** `TalkScreen.ItemLine` shows "dal: 1 katori" and the band; `plate_unit_taken_as` has no caller. | **CONDITIONAL**: true when Ira's plate card renders the caption; on watch in `countdown.md`; not rendered by the evening of 25 Sep, the caption comes off the mockups (Vedant). The deck's only conditional claim as of 23:17. |
