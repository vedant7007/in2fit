...
| No absolute paths from anyone's machine | `git grep` for `%USERPROFILE%`, `AppData`, `<repo parent>`, container paths | **five hits, all the username `vedan` inside the SDK path** `%LOCALAPPDATA%\Android\Sdk`: `HANDOVER.md` §2 and §5, `docs/decisions/0003` (twice), `docs/review-packets/phase-1b.md`, and hard-coded in `tools/hardware-probe.ps1:24`. The documents describe the SDK choice and could say `%LOCALAPPDATA%\Android\Sdk` instead; the script should read `$env:LOCALAPPDATA` as `build-llama-android.ps1` does (Rao's file, one line). **Ruled 20 Sep, stripped at 19:50: every occurrence is now a placeholder (`%LOCALAPPDATA%`, `%USERPROFILE%`, `<main tree>`, `<repo parent>`), and `hardware-probe.ps1` reads `$env:LOCALAPPDATA`.** |
| No private network details | `git grep` for IP addresses | **one**: the phone's LAN address `<phone LAN address>:5555` (adb over Wi-Fi) in `COORDINATION.md`, twice. Harmless in itself; a home-network detail all the same. **Ruled 20 Sep, stripped at 19:50: `<phone LAN address>`.** |
