...
class DefaultOrchestratorTest {
    ...
    @Test fun `LOG resolves, saves, evaluates and phrases, in that order`() {
        ...
        // The resolved items ride the event, one per item in item order, with their grams (the "taken as" caption).
        assertEquals(listOf(150.0, 150.0), (events[resolvedAt] as OrchestratorEvent.MealResolved).items.map { it.snapshot.grams })
