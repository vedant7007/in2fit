...
object LogPrefilter {
    ...
    /**
     * True when the utterance says a food was NOT eaten: "no rice today", "didn't eat lunch",
     * "nahi khaya", "skipped breakfast". The counterweight to the quantity rule, ruled with it
     * (Vedant, 20 Sep): logging a meal the person just said they did not eat is the worst
     * version of a misroute, because they are actively telling us the opposite. A negation
     * blocks the words' LOG here and the model's LOG in [IntentRouter.accept].
     *
     * THE ONE EXCEPTION is a self-correction, which the demo's #4 IS: "two rotis no, three rotis
     * and dal", "दो रोटी नहीं, तीन रोटी और दाल". A negation followed directly by a quantity is
     * a correction of the count, not a denial of the food, and does not block.
     */
    fun negatesFood(transcript: String): Boolean {
        val text = FoodTextMatching.normalise(transcript)
        return text.isNotEmpty() && negates(text.split(' '))
    }

    private fun negates(words: List<String>): Boolean = words.indices.any { i ->
        words[i] in NEGATIONS && !(words.getOrNull(i + 1)?.let { it in QUANTITY_WORDS || it.all { c -> c.isDigit() } } ?: false)
    }
     ...
     * "three days no rice" is not logged: [NEGATIONS] is read first.
     */
    ...
    /**
     * Words that deny a food or a meal. Whole words after normalisation, which strips the
     * apostrophe: "didn't" arrives as "didn t", so the stems are listed. "no" and "not" are here
     * too; the self-correction exception in [negatesFood] keeps "two rotis no, three rotis".
     * Roman and Devanagari Hindi, roman and Telugu-script Telugu: GENERATED by someone who does
     * not read them, listed on `log-words-review.md`; Vedant verifies the Devanagari.
     */
    internal val NEGATIONS: Set<String> = setOf(
        "no", "not", "never", "nothing", "none", "without", "skip", "skipped", "skipping", "didn", "didnt", "don", "dont", "doesn", "doesnt",
        "hadn", "hadnt", "haven", "havent", "wasn", "wasnt", "couldn", "couldnt",
        // roman Hindi
        "nahi", "nahin", "nai", "nahee", "mat", "bina",
        // Devanagari, as the hi recogniser writes them; Vedant verifies
        "नहीं", "नही", "बिना", "छोड़", "छोड़ा", "छोड़ी",
        // roman Telugu, then Telugu script; GENERATED, unreviewed
        "ledu", "ledhu", "kadu", "kaadu", "tinaledu", "thinaledu", "లేదు", "కాదు", "తినలేదు",
    )
